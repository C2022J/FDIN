...
def train_and_evaluate(optimizer_name):
    # Load AlexNet model each time before training
    model = models.alexnet(weights=AlexNet_Weights.IMAGENET1K_V1)
    model.classifier[6] = nn.Linear(model.classifier[6].in_features, 10)  # Change the last layer to 10 classes
...
if __name__ == '__main__':
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('./data/alexnet_training_loss_comparison.png')
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('./data/alexnet_test_accuracy_comparison.png')
