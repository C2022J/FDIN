...
def train_and_evaluate(optimizer_name):
    # Load SENet model each time before training
    model = models.squeezenet1_1(weights=SqueezeNet1_1_Weights.IMAGENET1K_V1)  # Replace with SENet model
    model.classifier[1] = nn.Conv2d(512, 10, kernel_size=(1, 1), stride=(1, 1))  # Modify final layer for 10 classes
...
if __name__ == '__main__':
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('./data/senet_training_loss_comparison.png')
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('./data/senet_test_accuracy_comparison.png')
