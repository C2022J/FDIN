...
def calculate_ssim(img1,
                   ...
                   input_order='HWC',
                   test_y_channel=False):
    """Calculate SSIM (structural similarity) using the standard skimage library.

    This function is a standard implementation that handles both Tensor and Numpy inputs,
    and is robust to images of varying sizes.
    """
    ...
    # Pre-process images from Tensor to Numpy if necessary
    ...
    # Determine the data range for SSIM calculation
    max_value = 1. if np.max(img1) <= 1 else 255.

    # Dynamically calculate a suitable window size for robustness
    h, w = img1.shape[:2]
    win_size = min(h, w, 7)
    if win_size % 2 == 0:
        win_size -= 1

    # Calculate SSIM using the standard skimage library
    if test_y_channel:
        img1 = to_y_channel(img1.astype(np.uint8))
        img2 = to_y_channel(img2.astype(np.uint8))
        ssim = structural_similarity(
            img1, img2, data_range=max_value, win_size=win_size)
    else:
        # For RGB images (HWC format), the channel axis is 2
        ssim = structural_similarity(
            img1, img2, data_range=max_value, channel_axis=2, win_size=win_size)
