...
def paired_paths_from_lmdb(folders, keys):
    ...
    [Fix] 修复了文件名包含多个点（如 0.005）被错误截断的问题。
    """
    ...
    with open(osp.join(input_folder, 'meta_info.txt')) as fin:
        # [关键修改] 不要 split('.')，直接 strip() 去掉换行符即可
        # 因为 meta_info.txt 里存的已经是干净的 Key 了
        input_lmdb_keys = [line.strip() for line in fin]

    with open(osp.join(gt_folder, 'meta_info.txt')) as fin:
        # [关键修改] 同上
        gt_lmdb_keys = [line.strip() for line in fin]

    # 如果两个列表长度一致但顺序不同，尝试排序对比（可选）
    # if set(input_lmdb_keys) != set(gt_lmdb_keys): ...

    paths = []
    # 过滤掉空行
    input_lmdb_keys = [k for k in input_lmdb_keys if k]
