...
from basicsr.models.archs.fcib_mono import FCIB_Mono

class MonoFDIN(BaseModule):
    def __init__(self,
                 img_channels=3,
                 mid_channels=16,
                 middle_blk_num=1,
        ...
        for idx, num in enumerate(enc_blk_nums):
            ...
            if idx in enc_blk_fcib_idx:
                self.fcib.append(FCIB_Mono(chan))
            else:
    ...
    def forward(self, inp):
        """Forward function for monocular input."""
        B, C, H, W = inp.shape
        ...
        pred = self.restore_image_size(pred)
