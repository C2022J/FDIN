...
class StereoImageCleanModel(BaseModel):
    ...
    def pad_test(self, window_size):        
        ...
        if h % window_size != 0:
            mod_pad_h = window_size - h % window_size       # 8 - 550 % 8 = 8 - 6 = 2
        if w % window_size != 0:
            mod_pad_w = window_size - w % window_size       # 8 - 350 % 8 = 8 - 6 = 2
        img_left = F.pad(self.lq_left, (0, mod_pad_w, 0, mod_pad_h), 'reflect')   # 552 × 352 => 1104 × 704
        img_right = F.pad(self.lq_right, (0, mod_pad_w, 0, mod_pad_h), 'reflect')
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if save_img:
                ...
                imwrite(gt_img_left, save_gt_img_path)
            ...
            if with_metrics:
                ...
                if use_image:
                    for name, opt_ in opt_metric.items():
                        ...
                        self.metric_results[name + '_left'] += getattr(metric_module, metric_type)(sr_img_left, gt_img_left, **opt_)
                        self.metric_results[name + '_right'] += getattr(metric_module, metric_type)(sr_img_right, gt_img_right, **opt_)
                else:
                    for name, opt_ in opt_metric.items():
                        metric_type = opt_.pop('type')
                        self.metric_results[name+ '_left'] += getattr(metric_module, metric_type)(visuals['result_left'], visuals['gt_left'], **opt_)
                        self.metric_results[name+ '_right'] += getattr(metric_module, metric_type)(visuals['result_right'], visuals['gt_right'], **opt_)
